...
CERT_MASTER = [
    ...
    "토목기사", "건축기사", "정보통신기사", "전자기사",
    "SQLD", "ADsP",
    # 정규직 필기 가산(부록 A-1 전직렬 공통) — 출처: 교통공사 제2025-186호
    "변호사", "변리사", "공인회계사", "공인노무사", "세무사", "법무사",
    "철도교통관제사", "산업안전기사", "빅데이터분석기사", "산업안전산업기사",
    # 정규직 해당직렬 상위(기술사/기능장)
    "전기기술사", "기계기술사", "토목/건축기술사", "정보통신기술사", "정보관리기술사",
    # 운전직
    "제2종 전기차량 운전면허",
]
...
# ═════════════════════════════════════════════════════════════
# 9) 정규직 필기 가산점 엔진 (출처: 부록 A — 검증된 공고 규칙)
# ═════════════════════════════════════════════════════════════
# [근거] 부산교통공사 정규직 공고 제2025-186호 / 부산시설공단 제2024-98호.
#   이 규칙들은 실제 공고 원문에서 추출·검증된 값이며(핸드오프 부록 A),
#   임의 하드코딩이 아니다. 기관마다 가점율이 다르므로 institution으로 분기한다.
#   가장 중요한 함정: 교통공사 장애인 5% ≠ 시설공단 장애인 3%. 섞으면 틀린다.
#
# 중복 규칙(공고 원문): "가산항목 간·동일항목 내 중복 불가(가장 유리한 1개).
#   단 취업지원대상자 + 자격증은 합산." 필기 가점은 '매 과목 4할 이상' 득점자만.
#   가점합격률 상한 30%.

# 기관별 정규직 가산 자격증 — 부록 A. 기관마다 가점율·대상이 달라 절대 일반화 금지.
# 교통공사=제2025-186호, 시설공단=제2024-98호.
_REG_CERT_ALL = {
    "부산교통공사": {  # 전직렬 공통: 전문자격 5%, 철도관제·안전기사류 3%
        "변호사": 5, "변리사": 5, "공인회계사": 5, "공인노무사": 5, "세무사": 5, "법무사": 5,
        "철도교통관제사": 3, "산업안전기사": 3, "빅데이터분석기사": 3, "산업안전산업기사": 3,
    },
    "부산시설공단": {  # 변호사·회계사 등 전문자격 10% (철도관제 등 교통 전용 항목은 미적용)
        "변호사": 10, "변리사": 10, "공인회계사": 10, "공인노무사": 10, "세무사": 10, "법무사": 10,
    },
}
# 해당직렬 가산(직무 의존). 교통: 기술사 5%/기사·산업기사 3%/컴활1급 3%(운영).
# 시설: 기술사·기능장 10%/기사 5%(기술) — 세부 직렬 적용은 공고 재확인.
_REG_CERT_TRACK = {
    "부산교통공사": {
        "사무·행정": {"컴퓨터활용능력 1급": 3},
        "전기": {"전기기술사": 5, "전기기사": 3, "전기산업기사": 3},
        "기계": {"기계기술사": 5, "일반기계기사": 3, "공조냉동기계기사": 3},
        "토목·건축": {"토목/건축기술사": 5, "토목기사": 3, "건축기사": 3},
        "신호·통신": {"정보통신기술사": 5, "정보통신기사": 3},
        "전산": {"정보관리기술사": 5, "정보처리기사": 3, "정보처리산업기사": 3},
    },
    "부산시설공단": {
        "전기": {"전기기술사": 10, "전기기사": 5, "전기산업기사": 5},
        "기계": {"기계기술사": 10, "일반기계기사": 5, "공조냉동기계기사": 5},
        "토목·건축": {"토목/건축기술사": 10, "토목기사": 5, "건축기사": 5},
        "신호·통신": {"정보통신기술사": 10, "정보통신기사": 5},
        "전산": {"정보관리기술사": 10, "정보처리기사": 5, "정보처리산업기사": 5},
    },
}

# 기관별 장애인 가점율(공고 명시값) — 일반화 금지
_DISABLED_PCT = {"부산교통공사": 5, "부산시설공단": 3}
REG_BONUS_CAP = 30   # 가점합격률 상한(%)


def regular_bonus_score(track: str, certs: list | None, social: dict | None,
                        institution: str = "부산교통공사") -> dict:
    """정규직 필기 가산 비율을 공고 규칙대로 계산한다(확률·합격예측 아님 — 규정 적용).
       track: 직무 카테고리(사무·행정/전기/...). certs: 보유자격 리스트.
       social: {취업지원대상자:0|5|10, 장애인:bool}. institution: 기관(가점율 분기).
       반환: 적용 가산비율 + 어떤 항목이 적용됐는지 설명 + 출처."""
    have = set(certs or [])
    social = social or {}

    # 1) 자격증 가산: 전직렬 + 해당직렬 중 '가장 유리한 1개'(동일항목 내 중복 불가)
    cert_candidates = dict(_REG_CERT_ALL.get(institution, {}))
    cert_candidates.update(_REG_CERT_TRACK.get(institution, {}).get(track, {}))
    owned = [(c, p) for c, p in cert_candidates.items() if c in have]
    best_cert = max(owned, key=lambda t: t[1]) if owned else None
    cert_pct = best_cert[1] if best_cert else 0

    # 2) 전형 가산
    vet = int(social.get("취업지원대상자", 0) or 0)
    vet_pct = vet if vet in (5, 10) else 0
    dis_pct = _DISABLED_PCT.get(institution, 5) if social.get("장애인") else 0

    # 3) 중복 규칙: 보통 1개만, 단 (취업지원 + 자격증)만 합산 허용
    candidate_totals = {
        "취업지원+자격증": vet_pct + cert_pct,   # 유일하게 합산 허용
        "장애인": dis_pct,
    }
    chosen = max(candidate_totals, key=candidate_totals.get)
    pct = min(candidate_totals[chosen], REG_BONUS_CAP)
    capped = candidate_totals[chosen] > REG_BONUS_CAP

    notes = []
    if chosen == "취업지원+자격증":
        if vet_pct: notes.append(f"취업지원대상자 {vet_pct}%(보훈부 증명서 비율)")
        if best_cert: notes.append(f"{best_cert[0]} {best_cert[1]}%")
        if not notes: notes.append("적용 가능한 가산 없음")
    else:
        notes.append(f"장애인 {dis_pct}%({institution} 기준)")
    if capped:
        notes.append(f"※ 상한 {REG_BONUS_CAP}% 적용")

    return dict(
        기관=institution, 직무=track,
        가산비율=pct,
        적용항목=chosen if pct else None,
        근거=notes,
        주의=(f"필기 만점의 {pct}%. 단 '매 과목 4할 이상' 득점자만 적용. "
              f"가산항목 간 중복 불가(취업지원+자격증만 합산). 출처: "
              f"{'교통공사 제2025-186호' if institution=='부산교통공사' else '시설공단 제2024-98호'}. "
              "당해 공고로 항목·비율 재확인 필수."),
    )
